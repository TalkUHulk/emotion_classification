...
regu_losses = tf.reduce_mean(regu_losses)

total_loss = regu_losses + loss

correct_prediction = tf.equal(tf.argmax(logits, 1, output_type=labels.dtype), labels)
...
with tf.control_dependencies(update_ops):
    optimizer = tf.train.AdamOptimizer(lr, name='optimizer').minimize(total_loss, global_step=global_step)
...
tf.summary.scalar('regu_losses', regu_losses)
...
with tf.Session() as sess, open('train.log', 'w') as log:
    ...
    for n in range(epoch):
        for i in range(steps):
            ...
            g_step_, loss_, acc_, train_summary, _ = sess.run([global_step, total_loss, accuracy, merged, optimizer],
                                                              feed_dict={handle: train_handle, train_mode: True})
        ...
        acc_sum = .0
        for j in range(10):
            acc_ = sess.run(accuracy, feed_dict={handle: val_handle,  train_mode: False})
            acc_sum += acc_
